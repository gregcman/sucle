...
(defun ouprint2 (object)
  ...
  (setf *num-info* (splitter *differences* fooannot0))
  (let ((more-data (parallel-walk footrue *num-info* fooannot0)))
    (let ((len (length more-data)))
      (let ((data (make-array len :initial-element nil))
	    (bits (make-array len :element-type (quote bit))))
	(dotimes (index len)
	  (let ((value (aref more-data index)))
	    (if value
		(setf (aref data index) (aref obj-stream value))
...
(defun splitter (&optional (differences *differences*) (text fooannot0) (scratch *Stringer*))
  (let* ((len (length text))
	 (ids (make-array len :initial-element nil))
	 (stackno nil)
	 (index 0))
    (loop
       (when (>= index len)
	 ...
	 (case value
	   ...
	   (1 (setf (fill-pointer scratch) 0)
		 ...
		 (let ((value (aref differences index)))
		   (unless (zerop value)
		     (let ((value (read-from-string scratch)))
		       (push value stackno)
		       (setf (aref ids index) (cons :start value)))
		     (return))
		   (vector-push-extend (aref text index) scratch))))
	   (2 nil)
	   (3 (setf (aref ids index) (cons :end (pop stackno))))))
       (incf index))
    ids))
...
(defun parallel-walk (&optional (text footrue) (walk-info *num-info*) (annot fooannot0))
  (let* ((annot-index 0)
	 (text-index 0)
	 (len (length text))
