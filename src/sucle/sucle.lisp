(defpackage #:sucle
  (:use #:cl #:utility #:application #:control)
  (:export #:start))
(in-package :sucle)
...
(defparameter *sucle-app-function*
  (run-with
   (lambda ()
     #+nil
     (setf (sandbox-sub::entity-fly? *ent*) nil
	   (sandbox-sub::entity-gravity? *ent*) t)
     ;;(our-load)
...
(defun render? ()
  ...
  (when (sandbox-sub::fister-exists *fist*)
    (let ((selected-block (sandbox-sub::fister-selected-block *fist*)))
