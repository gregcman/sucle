...
(defclass crud-sqlite (crud)
  ((handles :initform (lparallel.queue:make-queue) :accessor handles)))
(defmacro with-crud-sqlite ((&optional (impl '*implementation*)) &body body)
  `(let ((database::*database* (path ,impl))
	 (database::*handles* (handles ,impl)))
     ,@body))

(defmethod crud_create (name data (impl crud-sqlite))
  (with-crud-sqlite (impl)
